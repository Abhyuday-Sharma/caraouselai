...
export function HighlightCard({ title, subtitle }) {
  return (
    <div className="slide-glass-card" style={{ 
      ...
      <h4 style={{ fontSize: '1.5em', margin: '0 0 0.25em 0', color: 'var(--sys-text)' }}>{title}</h4>
      {subtitle && <p style={{ fontSize: '1em', margin: 0, color: 'var(--sys-text-muted)', lineHeight: 1.5 }}>{subtitle}</p>}
    </div>
  );
}
...
export function ComparisonCard({ leftLabel, leftText, rightLabel, rightText }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1em', width: '100%' }}>
      <div className="slide-glass-card" style={{ ...cardStyle, borderTop: '4px solid #ef4444' }}>
        ...
        <p style={{ margin: 0, fontSize: '1.1em', lineHeight: 1.5 }}>{leftText}</p>
      </div>
      <div className="slide-glass-card" style={{ ...cardStyle, borderTop: '4px solid #22c55e' }}>
        ...
        <p style={{ margin: 0, fontSize: '1.1em', color: 'var(--sys-text)', lineHeight: 1.5 }}>{rightText}</p>
      </div>
    </div>
  );
}
...
export function ListBlock({ items }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1em', width: '100%' }}>
      {items.map((item, i) => (
        <div key={i} style={{ display: 'flex', gap: '1em', alignItems: 'center', background: 'rgba(255,255,255,0.05)', padding: '1em', borderRadius: 'var(--sys-radius-md)' }}>
          <CheckCircle2 style={{ color: 'var(--sys-accent)', flexShrink: 0 }} />
          <span style={{ fontSize: '1.1em', fontWeight: '500' }}>{item}</span>
        </div>
      ))}
    </div>
  );
